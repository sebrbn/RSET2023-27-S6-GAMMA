...
def run_demo():
    ...
    parser.add_argument("--model", type=str, default="sac_drl_final", help="Model name in models/ (without .zip)")
    parser.add_argument("--stage", type=int, default=3, help="Curriculum stage to run (1-3)")
    ...
    print(f"Starting DRL baseline demonstration (Stage {args.stage}). Press Ctrl+C to stop.")
    try:
        while True:
            obs, _ = env.reset()
            terminated = False
